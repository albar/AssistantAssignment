...
{
    public class NormalizedAssessmentsValuesCalculator :
        ObjectiveValueCalculatorBase
    {
        public override Objectives Objective { get; } =
            Objectives.NormalizedAssessmentsValues;
                    ...
                    Task.Run(() =>
                    {
                        var ordered = chromosome.Phenotype.Select(phenotype =>
                            phenotype.AssesmentsValues[assesment])
                                .OrderBy(assesmentValue => assesmentValue)
                                .ToArray();
                        ...
                        return new KeyValuePair<Assesments, Func<double, double>>(
                            assesment,
                            value => Math.Abs(value - lowest) / range);
                    }, token));

            var normalizers = await Task.WhenAll(normalizerTasks);

            var tasks = chromosome.Phenotype.Select(phenotype =>
                Task.Run(() => normalizers.Select(normalizer =>
                    normalizer.Value.Invoke(
                        phenotype.AssesmentsValues[normalizer.Key])),
                token));
